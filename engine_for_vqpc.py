...
def train_batch_s2_VQPC(model, input_pc, gt_pc, radius):
    pred, CE_loss, loss_vq, loss_pretrain = model(input_pc, gt_pc)
    # cd_dist, hd_value = compute_cd_hd_distance(pred, gt_pc, lamda_cd=100, lamda_hd=1)
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                if stage == 'autoencoder':
                    ...
                    #     train_batch_s2_VQPC(model, input_data, gt_data, radius_data)
                    # loss, CE_loss, loss_vq, loss_pretrain = \
                    #     train_batch(model, input_data, gt_data, radius_data, stage) #, loss_pretrain
