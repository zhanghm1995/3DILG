...
class feature_extraction(nn.Module):
    def __init__(self, growth_rate=24, dense_n=3, knn=16, input_channel=256):
...
class Decoder(nn.Module):
    def __init__(self, up_ratio):  # params=None
        super(Decoder, self).__init__()
        self.feature_extractor = feature_extraction(growth_rate=24, dense_n=3, knn=16, input_channel=256)
        self.up_ratio = up_ratio  # params['up_ratio']
        # self.num_points=params['patch_num_point']
        # self.out_num_point=int(self.num_points*self.up_ratio)
        self.up_projection_unit = up_projection_unit()
        ...
        #     nn.Conv1d(in_channels=256, out_channels=self.up_ratio * 3, kernel_size=1),
        #     # nn.Conv1d(in_channels=256, out_channels=128, kernel_size=1),
        #     # nn.Conv1d(in_channels=128, out_channels=self.up_ratio * 3, kernel_size=1),
        #     nn.ReLU()
        # )
        self.conv1 = nn.Sequential(
            # nn.Conv1d(in_channels=128, out_channels=64, kernel_size=1),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=self.up_ratio * 3, kernel_size=1)  # 64
        )
    ...
    def forward(self, xyz, feature):
        ...
        :param xyz: [B, N, 3]
        :param feature: [B, C, N]
        :return: new_xyz: xyz + offset
        '''
        feature = self.feature_extractor(feature)  # b,648,n
        ...
        coord = xyz.repeat(1, 1, self.up_ratio) + feature
        # print(xyz.shape, coord.shape, feature.shape)
        # coord = coord.view(xyz.shape)
        coord = rearrange(coord, 'b n (r p) -> b (n r) p', p=3)
...
class up_projection_unit(nn.Module):
    def __init__(self, up_ratio=4):
        ...
        self.conv1 = nn.Sequential(
            Conv1d(in_channels=256, out_channels=128, kernel_size=1),  # 648
            nn.ReLU()
        )
        self.up_block1 = up_block(up_ratio=up_ratio, in_channels=128 + 2)
        self.up_block2 = up_block(up_ratio=up_ratio, in_channels=128 + 2)
        self.down_block = down_block(up_ratio=up_ratio, in_channels=128)
