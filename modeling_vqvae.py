...
class Decoder(nn.Module):
    def __init__(self, latent_channel=192):
        ...
        self.up_projection_unit = up_projection_unit(up_ratio=4)
    ...
    def forward(self, latents, centers):  # , samples
        '''
        :param latents: quantized feature z_q_x_st
        :param centers: centers
        :param samples: normals
        :return:
        '''
        # kernel average
        # samples: B x N x 3  #Bx512x3
        # latents: B x T x 320
        # centers: B x T x 3

        embeddings = embed(centers, self.basis)
        embeddings = self.embed(torch.cat([centers, embeddings], dim=2))
        latents = self.transformer(latents, embeddings)  # torch.Size([B, C=256, N=1024/r])
        latents = self.up_projection_unit(latents)  # [B,128,rN]
        latents = self.conv1(latents)
        latents = self.conv2(latents)  # [B,3,rN]
        prediction = latents.permute(0, 2, 1).contiguous()
...
class Autoencoder(nn.Module):
    def __init__(self, N, K=1024, dim=256, M=1024):
        super().__init__()
    ...
    def forward(self, x):
        ...
        prediction = self.decoder(z_q_x_st, centers)

        return prediction, z_e_x, z_q_x, loss_vq, perplexity
...
@register_model
def vqvae_256_1024_1024(pretrained=False, **kwargs):
    model = Autoencoder(
        N=256,
        K=1024,
        dim=256,
