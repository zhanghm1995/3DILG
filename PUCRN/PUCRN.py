...
class CRNet(torch.nn.Module):
    ...
    def __init__(self, up_ratio):
        ...
        step_up_rate = int(np.sqrt(up_ratio))
        self.upsampling_stage_1 = SubNetwork1(up_ratio=step_up_rate)
        self.upsampling_stage_2 = SubNetwork(up_ratio=step_up_rate)
        self.refinement_stage = SubNetwork(up_ratio=1)
...
class SubNetwork1(nn.Module):
    ...
    def __init__(self, up_ratio=2):
        ...
        self.feature_extractor = Transformer_extractor1(128, 64)
        self.up_unit = Upsampling_unit(up_ratio=up_ratio)
        self.regressor = MLP_CONV(in_channel=128, layer_dims=[64, 3])
    ...
    def forward(self, points, centers):
        ...
        offest = self.regressor(up_feat)
        # print('================',duplicated_point.size(), offest.size())
        up_point = duplicated_point + torch.tanh(offest)
...
class Transformer_extractor1(nn.Module):
    """
    Point-wise feature extractor.
    ...
    def forward(self, points, centers):
        feature_1 = self.mlp_1(points)  # [B,C=128,N=256]
        global_feature = torch.max(feature_1, 2, keepdim=True)[0]
        feature_2 = torch.cat([feature_1, global_feature.repeat((1, 1, feature_1.size(2)))], 1)
        feature_3 = self.mlp_2(feature_2)
        '''
        ...
        '''
        # print('==============', feature_3.size(), centers.size())#torch.Size([32, 256, 256]) torch.Size([32, 3, 256])
        point_feat = self.point_transformer(feature_3, centers)
        return point_feat
...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        key = grouping_operation(key.float().contiguous(), idx_knn)  # b, dim, n, n_knn
        qk_rel = query.reshape((b, -1, n, 1)) - key
        pos_rel = pos.reshape((b, -1, n, 1)) - grouping_operation(pos.float().contiguous(), idx_knn)  # b, 3, n, n_knn
