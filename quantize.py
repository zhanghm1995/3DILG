...
class VectorQuantizer_PointCloud(nn.Module):
    """
    Improved version over VectorQuantizer, can be used as a drop-in replacement. Mostly
    avoids costly matrix multiplications and allows for post-hoc remapping of indices.
    """
    ...
    def forward(self, z, temp=None, rescale_logits=False, return_logits=False, top_k=False, k=10):
        assert temp is None or temp == 1.0, "Only for interface compatible with Gumbel"
        assert rescale_logits == False, "Only for interface compatible with Gumbel"
        assert return_logits == False, "Only for interface compatible with Gumbel"
        # reshape z -> (batch, height, width, channel) and flatten
        z = rearrange(z, 'b c n -> b n c').contiguous()
        z_flattened = z.view(-1, self.e_dim)  # [BN, C]
        ...
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            ...
            torch.einsum('bd,dn->bn', z_flattened, rearrange(self.embedding.weight, 'n d -> d n'))
        if top_k:
            _, min_encoding_top_k_list = torch.topk(d, 10, dim=1, largest=False)
            print(type(min_encoding_top_k_list), min_encoding_top_k_list.size(), min_encoding_top_k_list)
            indices = torch.tensor(random.sample(range(10), k))
            min_encoding_indices = min_encoding_top_k_list[:, torch.randint((min_encoding_top_k_list).size()[0])]
        else:
            min_encoding_indices = torch.argmin(d, dim=1)
        z_q = self.embedding(min_encoding_indices).view(z.shape)
        perplexity = None
